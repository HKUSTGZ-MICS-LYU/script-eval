...
set total = 1024        # 总任务数
set max_parallel = 32   # 最大并行数
set current = 0         # 起始任务编号
set lock_dir = "/nfs/share/home/tongliu/locks"  # 锁目录（使用PID防冲突）
set log_file = "/nfs/share/home/tongliu/parallel.log"  # 日志文件
# 初始化日志系统
...
while ($current < $total)
    ...
    if ($active_locks < $max_parallel) then
        ...
        while ($needed > 0 && $current < $total)
            ...
            (echo "[START] 任务 $current " >> $log_file ; bsub -n 4 -Is ./build2.sh VexiiRiscv $current 100  ; rm -rf $task_lock ; echo "[END]   任务 $current " >> $log_file ) &
            @ current++
            @ needed--
        end
    else
