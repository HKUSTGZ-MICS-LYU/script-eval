...
set log_file = "/nfs/share/home/tongliu/processordataset/script-eval/parallele-2G.log"  # 日志文件
# 初始化日志系统
...
while ($current < $total)
    ...
    if ($active_locks < $max_parallel) then
        ...
        while ($needed > 0 && $current < $total)
            ...
            (echo "[START] 任务 $current " >> $log_file ; bsub -n 4 -Is ./build2.sh RocketTile $current 2000 1 ; rm -rf $task_lock ; echo "[END]   任务 $current " >> $log_file ) &
            @ current++
            @ needed--
        end
    else
