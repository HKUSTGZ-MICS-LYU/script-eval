...
################################################################################
# 尝试从环境变量或参数获取 PROCESS_NODE，如果未设置则默认为 "tsmc6"
# 运行方式示例: 
# 1. set PROCESS_NODE "tsmc6"; source project_setup.tcl
# 2. icc2_shell -x "set PROCESS_NODE tsmc7; source project_setup.tcl"

if {![info exists PROCESS_NODE]} {
    set PROCESS_NODE "tsmc6"
    puts "INFO: No PROCESS_NODE specified. Defaulting to $PROCESS_NODE."
}

puts "INFO: Loading configuration for process node: $PROCESS_NODE"
...
switch $PROCESS_NODE {
    ...
    "tsmc6" {
        puts "INFO: Sourcing Original TSMC 6nm Configuration..."
        
        # ======================================================================
        # ORIGINAL CONTENT START (TSMC 6nm)
        # ======================================================================
        set TECH_ROOT           ""
        set LIB_ROOT            ""
    ...
    "tsmc7" {
        # Placeholder for TSMC 7nm
        puts "INFO: Loading TSMC 7nm Configuration..."
        # Add TSMC 7nm specific variables here (copy the structure from above)
        error "ERROR: TSMC 7nm configuration not yet implemented."
    }

    default {
        error "ERROR: Unknown process node '$PROCESS_NODE'. Supported: tsmc6, tsmc7"
    }
}